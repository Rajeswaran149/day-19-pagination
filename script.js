...
containerdiv.classList.add("container",'text-center', 'table-responsive');
...
table.classList.add("table",'table-borderd', 'table-hover' ,'table-dark');
...
function pageButtons(pages){
    ...
    for (var page = maxLeft; page <= maxRight; page++) {
    	wrapper.innerHTML += `<button value=${page} class="page btn btn-md  btn-outline-info">${page}</button>`
    }

    if (state.page != 1) {
        wrapper.innerHTML = `<button value=${1} class="page btn btn-md  btn-outline-info">&#171; First</button>` + wrapper.innerHTML
    }

    if (state.page != pages) {
        wrapper.innerHTML += `<button value=${pages} class="page btn btn-md  btn-outline-info">Last &#187;</button>`
    }
